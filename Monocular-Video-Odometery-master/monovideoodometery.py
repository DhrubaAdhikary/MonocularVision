...
class MonoVideoOdometery(object):
    ...
    def process_frame(self):
        ...
        if self.id < 2:
            # --- MODIFIED LINES ---
            old_frame_path = os.path.join(self.file_path, str(0).zfill(6)+'.png')
            current_frame_path = os.path.join(self.file_path, str(1).zfill(6)+'.png')
            self.old_frame = cv2.imread(old_frame_path, 0)
            self.current_frame = cv2.imread(current_frame_path, 0)
            # --- END MODIFICATIONS ---
            
            # Add a check to ensure images were loaded
            assert self.old_frame is not None, f"Could not read image at {old_frame_path}"
            assert self.current_frame is not None, f"Could not read image at {current_frame_path}"
            ...
            self.old_frame = self.current_frame
            # --- MODIFIED LINE ---
            current_frame_path = os.path.join(self.file_path, str(self.id).zfill(6)+'.png')
            self.current_frame = cv2.imread(current_frame_path, 0)
            # --- END MODIFICATION ---
