...
# --- MODIFIED: Cleaned up and corrected all Bokeh imports ---
from bokeh.io import output_file, show
from bokeh.plotting import figure, ColumnDataSource
from bokeh.layouts import column, gridplot
from bokeh.models import Div, Panel, Tabs, WheelZoomTool
...
def visualize_paths(gt_path, pred_path, html_tile="", title="VO exercises", file_out="plot.html"):
    ...
    fig1 = figure(title="Paths", tools=tools, match_aspect=True, sizing_mode="stretch_width", toolbar_location="above",
                  x_axis_label="x", y_axis_label="y")
    
    # --- MODIFIED: Added the `size` parameter ---
    fig1.circle("gtx", "gty", size=4, source=source, color="blue", hover_fill_color="firebrick", legend_label="GT")
    fig1.line("gtx", "gty", source=source, color="blue", legend_label="GT")

    # --- MODIFIED: Added the `size` parameter ---
    fig1.circle("px", "py", size=4, source=source, color="green", hover_fill_color="firebrick", legend_label="Pred")
    fig1.line("px", "py", source=source, color="green", legend_label="Pred")

    fig1.multi_line("disx", "disy", source=source, legend_label="Error", color="red", line_dash="dashed")
    fig1.legend.click_policy = "hide"

    fig2 = figure(title="Error", tools=tools, sizing_mode="stretch_width", toolbar_location="above",
                  x_axis_label="frame", y_axis_label="error")
                  
    # --- MODIFIED: Added the `size` parameter ---
    fig2.circle("diffx", "diffy", size=4, source=source, hover_fill_color="firebrick", legend_label="Error")
    fig2.line("diffx", "diffy", source=source, legend_label="Error")
...
def plot_residual_results(qs_small, small_residual_init, small_residual_minimized,
    ...
    # --- MODIFIED: Replaced the generic `layout()` function with the more explicit `column()` ---
    plot_layout = column(
        Div(text="<h1>Bundle Adjustment exercises</h1>"),
        Div(text="<h2>Bundle adjustment with reduced parameters</h2>"),
        gridplot([[fig1, fig2, fig3]], toolbar_location='above'),
        Div(text="<h2>Bundle adjustment with all parameters (with sparsity)</h2>"),
        gridplot([[fig4, fig5, fig6]], toolbar_location='above')
    )
    show(plot_layout)
